...
public class TestingSimulation {


    // Need a method for perfect square:
    public static void tryPerfectSquare(int N) throws InputMismatchException {
        if (N < 1 || N > 625 || Math.sqrt(N) != ((double) ((int) Math.sqrt(N))))
            throw new InputMismatchException("Invalid input. Enter a perfect square between 1 and 625");
    }

    // Need a method to color Letters:
    ...
    // Find the position of patient zero:
    ...
    public static char[][] timeStep(char[][] patients, int size, double alpha, double beta) {
        //Create number of infected and recovered individuals count and ratio of infected
        //individuals to total # of individuals
        int numInfected = 0;
        int numRecovered = 0;
     ...
     * It takes the current patient grid and saves it into a file. The files are stored on the replit folder "infectionGrids".
    ...
    public static void fileWriter(char[][] patients, int fileCount, int size) throws FileNotFoundException {
        ...
        PrintWriter outputFile = new PrintWriter("infectionGrids/infectionGrid" + (fileCount+1) + ".txt");
    ...
    public static void main(String[] args) {
        ...
        do {
            try { // File exception handling.
                System.out.println("Time step: " + (currentTimeStep+1));
